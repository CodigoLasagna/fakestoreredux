...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        const product = req.body; // Espera un único producto
        ...
        try {
            ...
            return res.status(200).json({ message: 'Producto procesado correctamente.' });
        } catch (error) {
            console.error(error); // Agregar un log para ayudar a depurar
